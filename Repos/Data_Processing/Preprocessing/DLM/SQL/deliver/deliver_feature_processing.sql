...
--成交数据统计
DROP TABLE IF EXISTS marketing_modeling.dlm_tmp_deliver_processing;
CREATE TABLE marketing_modeling.dlm_tmp_deliver_processing as  
SELECT
    a.mobile,
	a.deal_time,    -- 首次成交时间
	a.last_time,    --最后成交时间   
...
FROM
	(
		...
		FROM
			marketing_modeling.dlm_tmp_deliver_cleansing2 
		WHERE rn = 1
	) a 
	left join 
	(
