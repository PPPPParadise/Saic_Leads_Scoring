...
DROP TABLE IF EXISTS marketing_modeling.dlm_tmp_feature_join_1;
CREATE TABLE marketing_modeling.dlm_tmp_feature_join_1 as 
SELECT 
	a1.mobile,
...
FROM
	...
	left join 
	marketing_modeling.dlm_tmp_leads_processing a on a1.mobile = a.mobile 
	left join 
	marketing_modeling.dlm_tmp_cust_processing b on a1.mobile = b.mobile 
    left join 
	marketing_modeling.dlm_tmp_hall_flow_processing c on a1.mobile = c.mobile
	left join 
	marketing_modeling.dlm_tmp_followup_processing d on a1.mobile = d.mobile 
	left join 
	marketing_modeling.dlm_tmp_receive_processing e on a1.mobile = e.mobile
	left join 
	(
		select * from marketing_modeling.dlm_tmp_oppor_processing
	...
	left join 
	marketing_modeling.dlm_tmp_deliver_processing a4 on a1.mobile = a4.mobile 
	left join	
	marketing_modeling.dlm_tmp_fail_processing a5 on a1.mobile = a5.mobile 
	left join	
	marketing_modeling.dlm_tmp_oppor_processing2 a6 on a1.mobile = a6.mobile
	left join	
	marketing_modeling.dlm_tmp_deal_flag_feature a7 on a1.mobile = a7.mobile
;
