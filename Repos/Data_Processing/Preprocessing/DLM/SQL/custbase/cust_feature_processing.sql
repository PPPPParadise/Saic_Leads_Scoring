...
DROP TABLE IF EXISTS marketing_modeling.dlm_tmp_cust_processing;
CREATE TABLE marketing_modeling.dlm_tmp_cust_processing as  
SELECT
    a.mobile,
...
FROM
	(
		SELECT
			...
			age
		FROM
			marketing_modeling.dlm_tmp_cust_cleansing 
		WHERE rn = 1
	) a 
	left join 
	(
