...
hive -hivevar pt=$pt -hivevar beginTime=$beginTime -hivevar queuename=$queuename -f deliver_feature_cleansing.sql
if [ $? -ne 0 ];then 
    echo "处理deliver_feature_cleansing.sql 失败。code=$?";
    exit 1; 
else
    echo "==========================deliver_feature_processing.sql start.2.....";
fi
hive -hivevar queuename=$queuename -f deliver_feature_processing.sql