...
hive -hivevar pt=$pt -hivevar beginTime=$beginTime -hivevar queuename=$queuename -f leads_feature_cleansing.sql
if [ $? -ne 0 ];then 
    echo "leads_feature_cleansing.sql 处理失败。code=$?";
    exit 1; 
else
    echo "=============================leads_feature_processing.sql start.2....."
fi
hive -hivevar queuename=$queuename -f leads_feature_processing.sql