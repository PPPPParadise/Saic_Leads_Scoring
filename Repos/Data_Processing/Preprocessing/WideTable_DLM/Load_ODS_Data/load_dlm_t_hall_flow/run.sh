...
hive -hivevar pt=$pt -hivevar beginTime=$beginTime -hivevar queuename=$queuename -f hall_flow_feature_cleansing.sql
if [ $? -ne 0 ];then 
    echo "hall_flow_feature_cleansing.sql 处理失败。code=$?";
    exit 1; 
else
    echo "=================================hall_flow_feature_processing.sql start.2.....";
fi
hive -hivevar queuename=$queuename -f hall_flow_feature_processing.sql
