...
SELECT 
	id,
	-- '%MG是否注册APP'
	max(case when father_id='261993005056' then score end) as is_app_user,
	max(case when display like '%MG最后一次浏览官网日期%' then substr(cast(cast(score as bigint) as string),0,8) end) as last_browse_time,
	max(case when display like '%MG过去14天浏览官网页面次数' then cast(score as int) end) as brwose_count_d14,
	max(case when display like '%MG过去14天访问APP平均时长' then score end) as avg_browse_time_d14,
	max(case when display like '%MG最近一次访问APP时间' then substr(cast(cast(score as bigint) as string),0,8) end) as last_app_visit_time,
	max(case when display like '%MG过去14天取得积分总数' then score end) as app_point_ttl_d14,
	max(case when display like '%MG总使用积分次数' then score end) as app_point_ttl,
	max(case when display like '%MG最近一次使用积分时间' then cast(score as int) end) as last_app_point_use_time,
	--display like '%MG是否试乘试驾'
	max(case when tag_name like '%是否试乘试驾%' and tag_name like '%MG%' then score end) as is_trail_user,
	-- '试驾车系' --532575944704
	...
	max(case when display like '%MG总到店次数' then score end) as visit_count,
	max(case when display like '%MG过去14天到店次数' then score end) as visit_count_d14,
	...
	max(case when display like '%MG过去14天预约试乘试驾次数' then score end) as trail_count_d14,
	...
	max(case when display like '%MGAPP下订日期' then substr(cast(cast(score as bigint) as string),0,8) end) as order_time,
	max(case when display like '%MG上次外呼结果' then score end) as last_sis_result,
	max(case when display like '%MG是否打开短链' then score end) as open_link,
	...
	max(case when display like '%MG跨店建卡数量' then score end) as create_card_count,
	max(case when display like '%MG客户寿命' then score end) as cust_live_period,
	max(case when display like '%MG历史跟进次数' then score end) as followup_ttl,
	...
	max(case when display like '%MG战败核实状态' then score end) as fail_status,
	max(case when display like '%MGRX5 MAX视频观看记录' then score end) as rx5_video_time,
	max(case when display like '%MGMARVELX视频观看记录' then score end) as marvel_video_time,
	...
	max(case when display like '%MG最近流失预警类型' then score end) as last_lost_type,
	max(case when display like '%MG最近流失预警日期' then substr(cast(cast(score as bigint) as string),0,8) end) as last_lost_time,
	max(case when display like '%MG是否浏览爱车展厅了解详情车型及配置页' then score end) as roewe_brwose_homepage,
	max(case when display like '%MG是否搜索过车型关键词' then score end) as search_vel,
	max(case when display like '%MG是否观看过车型的资讯' then score end) as browse_model_info,
	max(case when display like '%MG是否预约过车型试驾' then score end) as trail,
	max(case when display like '%MG是否下订' then score end) as make_deal,
	max(case when display like '%MG关注车系' then score end) as model,
	...
	max(case when display like '%MG战败核实结果' then score end) as fail_real_result,
	max(case when display like '%MG过去14天浏览APP次数' then score end) as app_browse_count_d14,
	concat_ws('|', collect_set(case when tag_name like '%微信订阅号状态%' and tag_name like '%MG%' then display end)) as wechat_follower,
	max(case when display like '%MG已注册APP' then 1 end) as register_app
FROM 
